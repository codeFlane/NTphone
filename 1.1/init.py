...
print('[cyan]NTphonePy --1.1--')
#print('[blink][red on white]!!!NTphonePy находится в бета тестировании (1.1)')
...
while True:
	...
	if cmd == 'exit':
		...
		if brouser:
			...
			print('[dim green]v.0.1  16 456 байт')
			c = 1
		else:
		...
		while acmd != 'phone':
			...
			if acmd == 'phone':
				...
				print('[green]-brouser     [cyan]-lenning     [magenta]-musicman')
				xacmd = input('>>>')
				if xacmd == 'brouser':
		...
		print('[magenta]2] /+ost    -"/"')
		print('[bold black]3] classes  -clas (1.2)')
		acmd = input('>>')
		if acmd == 'nums':
		...
		print('[green on white]NTphonePy 1.1 _  286 строк  _ дата выхода: 27.10.22 _  3 974 байт (init:3 144 байт  filesmanger: 788 байт  opent: 1 027 байт  dop_data(store): 24 046 байт)')
