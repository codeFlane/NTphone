...
while ocmd != './exit':
	...
	if ocmd == './exit':
		...
		'''b = time()
								...
								e = int(c - d)
								sec = d
								if int(calindar.microsec) + int(e) > 1000:
									d = d + 1
									micr = calindar.microsec + e - 1000
								else:
									micr = int(calindar.microsec) + e
								mn = calindar.minute
								while sec > 60:
									sec = sec - 60
									mn = mn + 1
								...
								print('[bold black]program for NTphonePy')
								print(f'Год: {calindar.year}')
								print(f'Месяц: [cyan]{calindar.month_(calindar.month)}')
								print(f'День недели: [cyan]{calindar.week}')
								print(f'День: {calindar.day}')
								print(f'Час: {calindar.hour}')
								print(f'Минута: {mn}')
								print(f'Секунда: {sec}')
								print(f'Микросекунда: {micr}')'''