...
while True:
	...
	print('[cyan] 4] Info            -info')
	print('[blue] 5] Store           -store')
	print('[magenta] 6] Animation       -anim')
	print('[dim magenta]7][bold] Quit            -exit')
	print('[bold black] 8]Notepad (in 1.3)')
	cmd = input('>')
	if cmd == 'exit':
		break
	if cmd == 'anim':
		...
		if acmd == 'clas':
			...
			print('Введите число (до 999 миллиардов):')
			xacmd = input('>>>')
			if len(xacmd) < 4:
				print(xacmd)
			elif len(xacmd) < 7:
	...
	if ext:
		...
		print('[green on white]NTphonePy 1.2 _  423 строк  _ дата выхода: 29.10.22 _ (init:15 241 байт  filesmanger: 788 байт  opent: 1 027 байт  animations: 335 767 байт  dop_data(store): 32 377 байт)  __pycache__: 15 709 байт')
		print('[blue]Вес NTphonePy1.2: 371 491 байт')
